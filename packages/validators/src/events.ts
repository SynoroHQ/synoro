...
export const CurrencyCode = z.enum([
  "RUB", "USD", "EUR", "GBP", "CNY", "JPY", "KZT", "BYN", "UAH"
]);

// Base schemas for reusable components
const uuidSchema = z.string().uuid("Invalid UUID format");
const timestampSchema = z.date().or(z.string().datetime());
const tagSchema = z.string().min(1).max(50).regex(/^[a-zA-Zа-яА-Я0-9_-]+$/, "Invalid tag format");
const amountSchema = z.number().positive("Amount must be positive").max(999999999, "Amount is too large");
...
export const eventQuerySchema = z.object({
  ...
  // Sorting
  sortBy: z.enum(["timestamp", "amount", "title", "category"]).default("timestamp"),
  sortOrder: z.enum(["asc", "desc"]).default("desc"),
});

// Bulk operations
export const bulkDeleteEventsSchema = z.object({
  eventIds: z.array(uuidSchema).min(1, "At least one event ID is required").max(100, "Too many events"),
});
...
export const processReceiptSchema = z.object({
  ...
  ocrProvider: z.enum(["tesseract", "google", "aws", "azure"]).default("tesseract"),
...
export const eventAnalyticsSchema = z.object({
  ...
  metrics: z.array(z.enum(["count", "sum", "average", "trend"])).default(["count", "sum"]),
