...
import { categories, productCategories } from "@synoro/db/schema";

// Функция для получения всех родительских категорий
export async function getAllParentCategoryIds(
  db: any,
  categoryId: string,
): Promise<string[]> {
...
export async function recalculateCategoryProductsCount(
  db: any,
  categoryIds: string[],
): Promise<void> {
  ...
  try {
    ...
    console.error(
      "Ошибка при пересчете количества продуктов в категориях:",
      error,
    );
    throw error;
  }
...
export async function recalculateAllCategoryProductsCount(
  db: any,
): Promise<void> {
