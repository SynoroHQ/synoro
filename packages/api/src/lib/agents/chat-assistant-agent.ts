  ...
  async canHandle(task: AgentTask): Promise<boolean> {
    try {
      ...
      const { object: chatAnalysis } = await generateObject({
        ...
        system: `Ты - эксперт по определению типов сообщений для чата в системе Synoro AI.

ТВОЯ ЗАДАЧА:
Определи, является ли сообщение пользователя обычным общением, не требующим специальной обработки.

ТИПЫ СООБЩЕНИЙ ДЛЯ ЧАТА:
- greeting: приветствия, здорования
- farewell: прощания, до свидания
- gratitude: благодарности, спасибо
- casual_chat: обычное общение, как дела
- small_talk: короткие беседы, поддержка
- other: прочие сообщения для общения

ПРИЗНАКИ СООБЩЕНИЙ ДЛЯ ЧАТА:
- Содержит приветствия, прощания, благодарности
- Короткие сообщения без конкретных запросов
- Общие вопросы о самочувствии, настроении
- Не требует специальной обработки или анализа
- Цель - поддержать общение, а не решить задачу

ПРАВИЛА:
1. Если сообщение для общения - это чат-сообщение
2. Если это конкретный запрос или вопрос - не чат
