...
import { EventProcessorAgent } from "../../src/lib/agents/event-processor-agent";
...
describe("EventProcessorAgent", () => {
  describe("Financial Extraction", () => {
    ...
    describe("Ruble amounts", () => {
      it("should extract amount with space as thousands separator", async () => {
        const result = await testFinancialExtraction(
          "Купил продукты за 1 299,90 ₽",
        );
        expect(result).toEqual({ amount: 1299.9, currency: "RUB" });
      });

      it("should extract amount with non-breaking space as thousands separator", async () => {
        const result = await testFinancialExtraction(
          "Потратил 1\u00A0299,90 ₽ на такси",
        );
        expect(result).toEqual({ amount: 1299.9, currency: "RUB" });
      });
      ...
      it("should extract amount with multiple spaces", async () => {
        const result = await testFinancialExtraction(
          "Стоимость: 1 000 000,50 ₽",
        );
        expect(result).toEqual({ amount: 1000000.5, currency: "RUB" });
      });
    ...
    describe("Dollar amounts", () => {
      it("should extract amount with space as thousands separator", async () => {
        const result = await testFinancialExtraction(
          "Paid $1 299.90 for the item",
        );
        expect(result).toEqual({ amount: 1299.9, currency: "USD" });
      });
    ...
    describe("Euro amounts", () => {
      it("should extract amount with space as thousands separator", async () => {
        const result = await testFinancialExtraction(
          "Paid €1 299,90 for the service",
        );
        expect(result).toEqual({ amount: 1299.9, currency: "EUR" });
      });
    ...
    describe("Edge cases", () => {
      it("should handle mixed separators", async () => {
        const result = await testFinancialExtraction(
          "Цена: 1 000,50 ₽ и $2,500.75",
        );
        // Должен вернуть первое найденное значение
        expect(result).toEqual({ amount: 1000.5, currency: "RUB" });
      });

      it("should handle very large numbers", async () => {
        const result = await testFinancialExtraction(
          "Стоимость проекта: 1 000 000 000,99 ₽",
        );
      ...
      it("should handle numbers with only decimal part", async () => {
        ...
        expect(result).toEqual({ amount: 0.5, currency: "RUB" });
      });
