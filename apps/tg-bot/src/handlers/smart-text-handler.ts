...
export async function handleSmartText(ctx: Context): Promise<void> {
  ...
  const processingMessageId = await sendProcessingMessage(ctx, processingMessage);
  ...
  try {
    ...
    if (shouldUseAgents) {
      ...
      result = await apiClient.messages.processMessageAgents.processMessageFromTelegramWithAgents.mutate({
        text,
        channel: "telegram",
      ...
      const { processTextMessage } = await import("../services/message-service");
    ...
    if (env.NODE_ENV === "development" && shouldUseAgents && "agentMetadata" in result && result.agentMetadata) {
...
function shouldUseAgentProcessing(text: string): boolean {
  ...
  const complexKeywords = [
    "анализ", "анализируй", "статистика", "сравни", "найди паттерн",
    "оптимизируй", "улучши", "план", "планирование", "стратегия",
    "исследование", "детальный", "подробный", "комплексный",
    "рекомендации", "предложения", "варианты", "альтернативы"
  ];
  ...
  const questionWords = ["что", "как", "где", "когда", "зачем", "почему", "какой", "кто"];
  const questionWordCount = questionWords.filter(word => textLower.includes(word)).length;
  ...
  const financialAnalysisKeywords = [
    "расходы", "доходы", "бюджет", "экономия", "трата", "финансы",
    "сколько потратил", "сколько заработал", "финансовый план"
  ];
  
  if (financialAnalysisKeywords.some(keyword => textLower.includes(keyword))) {
    return true;
  }
  
  // 5. Запросы с несколькими действиями (содержит "и", "а также", "еще")
  const multiActionIndicators = [" и ", " а также ", " еще ", " также ", " плюс "];
  if (multiActionIndicators.some(indicator => textLower.includes(indicator))) {
    return true;
  }
