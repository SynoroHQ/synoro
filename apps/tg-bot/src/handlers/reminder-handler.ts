...
export class ReminderHandler {
  ...
  static async handleCallback(ctx: Context, data: string, userId: string) {
    try {
      if (data === "list_reminders") {
        ...
        const pageStr = data.split("_")[2];
        const page = pageStr ? Number.parseInt(pageStr) : 0;
   ...
   */
  private static async handleDeleteReminder(
    ctx: Context,
    reminderId: string,
    userId: string,
  ) {
   ...
   */
  private static async handleEditReminder(
    ctx: Context,
    reminderId: string,
    userId: string,
  ) {
